...
extension XMLHandler {
    ...
    static func getEventElement(fromRoot root: XMLDocument, xmlContext: XmlContext, forEventName eventName: String) -> XMLElement? {
        root.at_xpath("asnx:module/namedType[@name='\(eventName)']", namespaces: xmlContext.namespaces)?.parent
    }
    ...
    static func getEventDefinition(contractElement: XMLElement, eventElement: XMLElement, xmlContext: XmlContext) -> EventDefinition? {
        ...
        guard let eventName = eventElement.at_xpath("namedType", namespaces: xmlContext.namespaces)?["name"] else { return nil }
        let parameters = eventElement.xpath("namedType/sequence/element", namespaces: xmlContext.namespaces).compactMap { each -> EventParameter? in
